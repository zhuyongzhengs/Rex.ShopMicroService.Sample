...
const getTableData = () => {
	...
	let dataParam = _.cloneDeep(state.tableData.param);
	delete dataParam.organizationUnitId;
	organizationUnitRoleApi.getSelectOuRoleList(ouId, dataParam).then((result) => {
		state.tableData.total = result.totalCount;
		state.tableData.data = result.items;
		state.tableData.loading = false;
	}).catch((err: any) => {
		state.tableData.loading = false;
		console.error("查询选择组织[角色]列表出错：", err);
	});
};
