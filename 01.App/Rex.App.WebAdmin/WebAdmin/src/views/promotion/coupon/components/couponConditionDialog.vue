<template>
  <div class="coupon-condition-container">
    ...
    >
      <el-form
        ref="couponConditionDialogFormRef"
      ...
      >
        <el-row :gutter="10">
          <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
            <el-form-item label="优惠劵条件" prop="conditionType">
              <el-select
                v-model="couponConditionType"
                size="default"
                style="width: 100%"
                placeholder="请选择优惠劵条件"
          ...
          <el-col
            v-show="couponConditionType == 'GOODS_ALL'"
            :xs="24"
            :sm="24"
            :md="24"
            :lg="24"
          ...
          <el-col
            v-show="couponConditionType == 'GOODS_IDS'"
            :xs="24"
            :sm="24"
            :md="24"
            :lg="24"
          ...
          <el-col
            v-show="couponConditionType == 'GOODS_CATS'"
            :xs="24"
            :sm="24"
            :md="24"
            :lg="24"
          ...
          <el-col
            v-show="couponConditionType == 'GOODS_BRANDS'"
            :xs="24"
            :sm="24"
            :md="24"
            :lg="24"
          ...
          <el-col
            v-show="couponConditionType == 'ORDER_FULL'"
            :xs="24"
            :sm="24"
            :md="24"
            :lg="24"
          ...
          <el-col
            v-show="couponConditionType == 'USER_GRADE'"
            :xs="24"
            :sm="24"
            :md="24"
            :lg="24"
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button
            type="primary"
            @click="submitValidate(couponConditionDialogFormRef)"
            size="default"
...
<script setup lang="ts" name="couponConditionDialog">
import {
  nextTick,
  reactive,
  ref,
...
const couponConditionDialogFormRef = ref();
const couponConditionType = ref("");
const selectedGoodItems = ref<RowGoodsType[]>([]);
...
const conditionTypeChange = () => {
  state.ruleForm.code = couponConditionType.value;
  let parameters = "";
...
const formRules = reactive<FormRules>({
  conditionType: [
    {
      validator: (rule: any, value: any, callback: any) => {
        if (!couponConditionType.value) {
          callback(new Error("请选择优惠劵条件！"));
          return;
        }
        callback();
      },
  ...
  goodsIds: [
    {
      validator: (rule: any, value: any, callback: any) => {
        if (
          ...
          couponConditionType.value == "GOODS_IDS"
        ) {
  ...
  goodsIdNums: [
    {
      validator: (rule: any, value: any, callback: any) => {
        if (!state.goodsIdsParameter.nums && couponConditionType.value == "GOODS_IDS") {
  ...
  goodsCategorys: [
    {
      validator: (rule: any, value: any, callback: any) => {
        if (
          !state.goodsCatsParameter.catId &&
          couponConditionType.value == "GOODS_CATS"
        ) {
  ...
  goodsCategoryNums: [
    {
      validator: (rule: any, value: any, callback: any) => {
        if (!state.goodsCatsParameter.nums && couponConditionType.value == "GOODS_CATS") {
          callback(new Error("请输入商品数量！"));
          return;
        }
        callback();
  ...
  goodsBrands: [
    {
      validator: (rule: any, value: any, callback: any) => {
        if (
          ...
          couponConditionType.value == "GOODS_BRANDS"
        ) {
  ...
  money: [
    {
      validator: (rule: any, value: any, callback: any) => {
        if (
          !state.goodsOrderFullsParameter.money &&
          couponConditionType.value == "ORDER_FULL"
        ) {
  ...
  userGrade: [
    {
      validator: (rule: any, value: any, callback: any) => {
        if (
          ...
          couponConditionType.value == "USER_GRADE"
        ) {
...
// 打开弹窗
const openDialog = (couponId: string, row: RowPromotionConditionType) => {
  nextTick(() => {
    state.dialog.editId = "";
    resetForms();
    state.ruleForm.couponId = couponId;
    if (row) {
      state.dialog.editId = row.id;
      Object.assign(state.ruleForm, row);
      couponConditionType.value = state.ruleForm.code;
      conditionTypeRevert();
      state.dialog.title = "修改优惠劵条件";
      state.dialog.submitTxt = "修 改";
    } else {
      state.dialog.title = "新增优惠劵条件";
