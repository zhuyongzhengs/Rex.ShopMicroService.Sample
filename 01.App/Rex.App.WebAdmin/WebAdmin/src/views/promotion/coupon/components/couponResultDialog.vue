<template>
  <div class="coupon-result-container">
    <el-dialog
    ...
    >
      ...
      >
        <el-row :gutter="10">
          <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
            <el-form-item label="优惠劵结果" prop="resultType">
              <el-select
                v-model="couponResultType"
                size="default"
                style="width: 100%"
                placeholder="请选择优惠劵结果"
          ...
          <el-col
            v-show="couponResultType == 'GOODS_REDUCE'"
            :xs="24"
            :sm="24"
            :md="24"
            :lg="24"
          ...
          <el-col
            v-show="couponResultType == 'GOODS_DISCOUNT'"
            :xs="24"
            :sm="24"
            :md="24"
            :lg="24"
          ...
          <el-col
            v-show="couponResultType == 'GOODS_ONE_PRICE'"
            :xs="24"
            :sm="24"
            :md="24"
            :lg="24"
          ...
          <el-col
            v-show="couponResultType == 'ORDER_REDUCE'"
            :xs="24"
            :sm="24"
            :md="24"
            :lg="24"
          ...
          <el-col
            v-show="couponResultType == 'ORDER_DISCOUNT'"
            :xs="24"
            :sm="24"
            :md="24"
            :lg="24"
          ...
          <el-col
            v-show="couponResultType == 'GOODS_HALF_PRICE'"
            :xs="24"
            :sm="24"
            :md="24"
            :lg="24"
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button
            type="primary"
            @click="submitValidate(couponResultDialogFormRef)"
            size="default"
...
<script setup lang="ts" name="couponResultDialog">
...
// 定义变量内容
const couponResultDialogFormRef = ref();
const couponResultType = ref("");
...
const resultTypeChange = () => {
  state.ruleForm.code = couponResultType.value;
  let parameters = "";
  ...
  resultType: [
    {
      validator: (rule: any, value: any, callback: any) => {
        if (!couponResultType.value) {
          callback(new Error("请选择优惠劵结果！"));
          return;
        }
        callback();
      },
  ...
  goodsReduce: [
    {
      validator: (rule: any, value: any, callback: any) => {
        if (
          ...
          couponResultType.value == "GOODS_REDUCE"
        ) {
  ...
  goodsDiscount: [
    {
      validator: (rule: any, value: any, callback: any) => {
        if (
          ...
          couponResultType.value == "GOODS_DISCOUNT"
        ) {
  ...
  goodsOnePrice: [
    {
      validator: (rule: any, value: any, callback: any) => {
        if (
          ...
          couponResultType.value == "GOODS_ONE_PRICE"
        ) {
  ...
  goodsOrderReduce: [
    {
      validator: (rule: any, value: any, callback: any) => {
        if (
          ...
          couponResultType.value == "ORDER_REDUCE"
        ) {
  ...
  goodsOrderDiscount: [
    {
      validator: (rule: any, value: any, callback: any) => {
        if (
          ...
          couponResultType.value == "ORDER_DISCOUNT"
        ) {
          callback(new Error("折扣不能为空，并且不能小于1，不能大于10的数字！"));
          return;
        }
  ...
  goodsHalfPriceNum: [
    {
      validator: (rule: any, value: any, callback: any) => {
        if (
          ...
          couponResultType.value == "GOODS_HALF_PRICE"
        ) {
...
// 打开弹窗
const openDialog = (couponId: string, row: RowPromotionResultType) => {
  nextTick(() => {
    state.dialog.editId = "";
    resetForms();
    ...
    if (row) {
      ...
      couponResultType.value = state.ruleForm.code;
      resultTypeRevert();
      state.dialog.title = "修改优惠劵结果";
      state.dialog.submitTxt = "修 改";
    } else {
      state.dialog.title = "新增优惠劵结果";
