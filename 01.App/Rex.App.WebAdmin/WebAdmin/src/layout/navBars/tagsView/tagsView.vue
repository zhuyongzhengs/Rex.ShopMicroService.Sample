...
const isActive = (v: RouteItem) => {
	if (getThemeConfig.value.isShareTagsView) {
		...
		if ((v.query && Object.keys(v.query).length) || (v.params && Object.keys(v.params).length)) {
			...
			// https://gitee.com/zhuyongzhengs/Rex.ShopMicroService.Sample/issues/I51RS9
			return v.path === state.routePath;
		}
	}
};
...
const addTagsView = (path: string, to?: RouteToFrom) => {
	// 防止拿取不到路由信息
	nextTick(async () => {
		// 修复：https://gitee.com/zhuyongzhengs/Rex.ShopMicroService.Sample/issues/I3YX6G
		let item: RouteItem;
		...
		if (item?.isLink && !item.meta?.isIframe) return false;
...
// https://gitee.com/zhuyongzhengs/Rex.ShopMicroService.Sample/issues/I61VS9
...
// https://gitee.com/zhuyongzhengs/Rex.ShopMicroService.Sample/issues/I5K3YO
// https://gitee.com/zhuyongzhengs/Rex.ShopMicroService.Sample/issues/I61VS9
...
// 拖动问题，https://gitee.com/zhuyongzhengs/Rex.ShopMicroService.Sample/issues/I3ZRRI
