...
[DependsOn(
    typeof(PromotionServiceHttpApiModule),
...
public class PromotionServiceHttpApiHostModule : AbpModule
{
    public override void PreConfigureServices(ServiceConfigurationContext context)
    {
        ...
        //    {
        //        options.AddAudiences("PromotionService");
    ...
    private void ConfigureCapRabbitMqOptions(ServiceConfigurationContext context, IConfiguration configuration)
    {
        context.Services.AddCap(x =>
        {
            ...
            // 2、存储消息
            x.UseEntityFramework<PromotionServiceDbContext>();
            x.UseMySql(configuration.GetConnectionString("Promotions"));
    ...
    */
    ...
    private void ConfigureDistributedCacheOption(ServiceConfigurationContext context)
    {
        ...
        context.Services.AddStackExchangeRedisCache(options =>
        {
            ...
            options.InstanceName = $"{typeof(PromotionServiceHttpApiHostModule).Namespace}:";
        });
    ...
    private void ConfigureAuditing()
    {
        Configure<AbpAuditingOptions>(options =>
        {
            ...
            options.ApplicationName = "促销应用服务";
        });
    }
    ...
    private void ConfigureVirtualFileSystem(ServiceConfigurationContext context)
    {
        ...
        if (hostingEnvironment.IsDevelopment())
        {
            ...
            {
                options.FileSets.ReplaceEmbeddedByPhysical<PromotionServiceDomainSharedModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}05.Modules{Path.DirectorySeparatorChar}Rex.PromotionService{Path.DirectorySeparatorChar}Rex.PromotionService.Domain.Shared"));

                options.FileSets.ReplaceEmbeddedByPhysical<PromotionServiceDomainModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                    $"..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}05.Modules{Path.DirectorySeparatorChar}Rex.PromotionService{Path.DirectorySeparatorChar}Rex.PromotionService.Domain"));

                options.FileSets.ReplaceEmbeddedByPhysical<PromotionServiceApplicationContractsModule>(
                        Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}05.Modules{Path.DirectorySeparatorChar}Rex.PromotionService{Path.DirectorySeparatorChar}Rex.PromotionService.Application.Contracts"));

                options.FileSets.ReplaceEmbeddedByPhysical<PromotionServiceApplicationModule>(
                        Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}05.Modules{Path.DirectorySeparatorChar}Rex.PromotionService{Path.DirectorySeparatorChar}Rex.PromotionService.Application"));
            });
        }
    }
    ...
    private void ConfigureConventionalControllers()
    {
        ...
        {
            options.ConventionalControllers.Create(typeof(PromotionServiceApplicationModule).Assembly, optAction =>
            {
                optAction.RootPath = "promotion"; // 默认：app
                optAction.RemoteServiceName = "Promotion";
            });
        });
    }
    ...
    private static void ConfigureSwaggerServices(ServiceConfigurationContext context, IConfiguration configuration)
    {
        context.Services.AddAbpSwaggerGenWithOAuth(
            ...
            {
                    {configuration["AuthServer:DefaultScope"], "PromotionService API"}
            },
            options =>
            {
                options.SwaggerDoc("v1", new OpenApiInfo { Title = "PromotionService API", Version = "v1" });
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        ...
        app.UseRemoteErrorResponse(SystemStatusCode.Error.ToPromotionServicePrefix());
        ...
        {
            c.SwaggerEndpoint("/swagger/v1/swagger.json", "PromotionService API");
