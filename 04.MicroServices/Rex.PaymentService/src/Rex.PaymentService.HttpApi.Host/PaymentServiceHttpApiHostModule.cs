...
public class PaymentServiceHttpApiHostModule : AbpModule
{
    ...
    private void ConfigureCapRabbitMqOptions(ServiceConfigurationContext context, IConfiguration configuration)
    {
        context.Services.AddCap(x =>
        {
            ...
            x.UsePostgreSql(configuration.GetConnectionString("Payments"));
    ...
    private void ConfigureDistributedCacheOption(ServiceConfigurationContext context)
    {
        ...
        context.Services.AddStackExchangeRedisCache(options =>
        {
            ...
            options.InstanceName = $"{typeof(PaymentServiceHttpApiHostModule).Namespace}:";
        });
    }
    ...
    private void ConfigureAuditing()
    {
        Configure<AbpAuditingOptions>(options =>
        {
            ...
            options.ApplicationName = "支付应用服务";
        });
    }
            ...
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                options.FileSets.ReplaceEmbeddedByPhysical<PaymentServiceDomainSharedModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}05.Modules{Path.DirectorySeparatorChar}Rex.PaymentService{Path.DirectorySeparatorChar}Rex.PaymentService.Domain.Shared"));

                options.FileSets.ReplaceEmbeddedByPhysical<PaymentServiceDomainModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                    $"..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}05.Modules{Path.DirectorySeparatorChar}Rex.PaymentService{Path.DirectorySeparatorChar}Rex.PaymentService.Domain"));

                options.FileSets.ReplaceEmbeddedByPhysical<PaymentServiceApplicationContractsModule>(
                        Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}05.Modules{Path.DirectorySeparatorChar}Rex.PaymentService{Path.DirectorySeparatorChar}Rex.PaymentService.Application.Contracts"));

                options.FileSets.ReplaceEmbeddedByPhysical<PaymentServiceApplicationModule>(
                        Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}05.Modules{Path.DirectorySeparatorChar}Rex.PaymentService{Path.DirectorySeparatorChar}Rex.PaymentService.Application"));
            });
        }
    }
    ...
    private void ConfigureConventionalControllers()
    {
        ...
        {
            options.ConventionalControllers.Create(typeof(PaymentServiceApplicationModule).Assembly, optAction =>
            {
                optAction.RootPath = "payment"; // 默认：app
                optAction.RemoteServiceName = "Payment";
            });
        });
    }
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        ...
        app.UseRemoteErrorResponse(SystemStatusCode.Error.ToPaymentServicePrefix());
