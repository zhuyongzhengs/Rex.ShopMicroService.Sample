...
using Rex.OrderService.MultiTenancy;
...
public class OrderServiceHttpApiHostModule : AbpModule
{
    ...
    private void ConfigureCapRabbitMqOptions(ServiceConfigurationContext context, IConfiguration configuration)
    {
        context.Services.AddCap(x =>
        {
            ...
            x.UsePostgreSql(configuration.GetConnectionString("Orders"));
    ...
    private void ConfigureDistributedCacheOption(ServiceConfigurationContext context)
    {
        ...
        context.Services.AddStackExchangeRedisCache(options =>
        {
            ...
            options.InstanceName = $"{typeof(OrderServiceHttpApiHostModule).Namespace}:";
        });
    }
    ...
    private void ConfigureAuditing()
    {
        Configure<AbpAuditingOptions>(options =>
        {
            ...
            options.ApplicationName = "订单应用服务";
        });
    }
            ...
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                options.FileSets.ReplaceEmbeddedByPhysical<OrderServiceDomainSharedModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}05.Modules{Path.DirectorySeparatorChar}Rex.OrderService{Path.DirectorySeparatorChar}Rex.OrderService.Domain.Shared"));

                options.FileSets.ReplaceEmbeddedByPhysical<OrderServiceDomainModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                    $"..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}05.Modules{Path.DirectorySeparatorChar}Rex.OrderService{Path.DirectorySeparatorChar}Rex.OrderService.Domain"));

                options.FileSets.ReplaceEmbeddedByPhysical<OrderServiceApplicationContractsModule>(
                        Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}05.Modules{Path.DirectorySeparatorChar}Rex.OrderService{Path.DirectorySeparatorChar}Rex.OrderService.Application.Contracts"));

                options.FileSets.ReplaceEmbeddedByPhysical<OrderServiceApplicationModule>(
                        Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}..{Path.DirectorySeparatorChar}05.Modules{Path.DirectorySeparatorChar}Rex.OrderService{Path.DirectorySeparatorChar}Rex.OrderService.Application"));
            });
        }
    }
    ...
    private void ConfigureConventionalControllers()
    {
        ...
        {
            options.ConventionalControllers.Create(typeof(OrderServiceApplicationModule).Assembly, optAction =>
            {
                optAction.RootPath = "order"; // 默认：app
                optAction.RemoteServiceName = "Order";
            });
        });
    }
    ...
    private static void ConfigureSwaggerServices(ServiceConfigurationContext context, IConfiguration configuration)
    {
        context.Services.AddAbpSwaggerGenWithOAuth(
            ...
            {
                    {configuration["AuthServer:DefaultScope"], "OrderService API"}
            },
            options =>
            {
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        ...
        app.UseRemoteErrorResponse(SystemStatusCode.Error.ToOrderServicePrefix());
