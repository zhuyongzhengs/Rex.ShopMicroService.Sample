...
namespace Rex.Shop.WebPublicGateway.Aggregations.Base;
...
public abstract class AggregateRemoteServiceBase<TDto> : IAggregateRemoteService<TDto>
{
        ...
        using (HttpClient httpClient = CreateHttpClient())
        {
            ...
            while (runningTasks.Count > 0)
            {
                ...
                try
                {
                    ...
                    _logger.LogInformation("本地化关键字：{0}，值：{1}", completedTask.Key, result);
                }
                catch (Exception ex)
                {
                    _logger.LogInformation("{0}错误：{1}", completedTask.Key, ex.Message);
                }
            }
        }
    ...
    public async Task<T> MakeRequestAsync<T>(HttpClient httpClient, string url)
    {
        ...
        {
            _logger.LogInformation("请求{0}:{1}出错", url, e.Message);
            throw;
        }
    }
